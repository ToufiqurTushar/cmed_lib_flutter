...
import 'family_profile_survey_argument.dart';
import 'dto/SurveyResultResponse.dart';


class FamilyProfileSurveyLogic extends BaseLogic {
  var allSurveys = <SurveyDto>[].obs;
  var selectedSurvey = Rxn<SurveyDto>();
  var selectedSurveyResult = Rxn<SurveyResultDto>();
  late FamilyProfileSurveyArgument familyProfileSurveyArgument;

  @override
  void onInit() {
    super.onInit();
    familyProfileSurveyArgument = (Get.arguments as FamilyProfileSurveyArgument);
    selectedSurvey.value = familyProfileSurveyArgument.selectedSurvey;
    fetchSurveyData();
  }
    ...
    httpProvider.POST(ApiUrl.postSurveyUrl(), surveyResultData.toJson()).then((response) {
      ...
      if (response.isOk) {
        ...
        AppDialogs.showSingleButtonDialog(centerImageUrl: 'assets/images/ic_success.svg', 'Survey Completed Successfully'.tr, positiveButtonText: 'OK'.tr, cancelable: false,onButtonClick:(){
          if(familyProfileSurveyArgument.redirectToServiceSelectionView??false) {
            Get.offNamedUntil(
        ...
        //   Get.offNamed(FamilyProfileSurveyResultView.routeName, arguments: FamilyProfileSurveyResultArgument(isFromHistory: false, selectedSurveyResult: surveyResultItemDto, selectedSurvey: selectedSurveyDto, customer: customer));
        // });
      } else {
